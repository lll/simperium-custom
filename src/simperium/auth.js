...
export class Auth extends EventEmitter {
   ...
   * @param {string} authUrl - Simperium.com application authentication URL
   */
  constructor(appId: string, appSecret: string, authUrl: string) {
    super();
    this.appId = appId;
  ...
  create(username: String, password: String, provider: ?String) {
    const userData: { username: String, password: String, provider?: String } =
      { username, password };
    if (provider) {
  ...
  request(endpoint: string, body: string): Promise<User> {
    const authUrl = `${this.authUrl}/${this.appId}/${endpoint}`;
...
export default (appId: string, appSecret: string, authUrl: string) => {
  return new Auth(appId, appSecret, authUrl);
};
